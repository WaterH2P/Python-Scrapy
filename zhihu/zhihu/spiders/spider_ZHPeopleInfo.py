...
class ZHPeopleInfoSpider(scrapy.Spider):
        ...
        if re.search(regex_1, response.url) is not None:
            ...
            try:
                userInfo = {
                    ...
                    'locations': [item['name'].replace('.', '').replace(',', '') for item in userJSON['locations'] if 'name' in item],
                    'educations': [{
                ...
                badges = []
                for info in userInfo['badge']:
                    if not isinstance(info, str):
                        if 'topics' in info:
                            for topic in info['topics']:
                                badges.append(str(topic['name'])+str(info['description']))
                        else:
                            badges.append(str(info['description']))
                if len(badges) > 0:
    ...
    def writeInfo(UId, userInfo):
        print('info ======')
        print(str(userInfo))
        with open('./User_Infos.txt', 'a', encoding='utf-8') as f:
            str_w = ''
            for key in userInfo:
